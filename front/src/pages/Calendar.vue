<template>
    <div>
      <div class="grid">
        ...
        <div class="col button">
          <Button @click="openCadastroModal" label="Cadastrar" class="custom-button" severity="info" />
        </div>
      </div>
      <DataTable
        :value="cars"
      ...
      >
        ...
        <Column header="Edit" :resizable="true">
          <template #body="{rowData}">
            <Button icon="pi pi-pencil" class="edit-button" @click="openEditModal(rowData)" />
          </template>
        </Column>
      </DataTable>
  
      <!-- Modal de Edição -->
      <Dialog v-model="displayEditModal" header="Editar Compromisso" :visible="displayEditModal" width="500px">
        ...
        <form @submit.prevent="saveChanges">
          ...
          <div class="p-dialog-footer">
            <Button label="Cancelar" icon="pi pi-times" class="p-button-text" @click="closeEditModal" />
            <Button type="submit" label="Salvar" icon="pi pi-check" class="p-button-text" />
          </div>
        </form>
      </Dialog>
  
      <!-- Modal de Cadastro -->
      <Dialog v-model="displayCadastroModal" header="Cadastrar Compromisso" :visible="displayCadastroModal" width="500px">
        ...
        <form @submit.prevent="saveNew">
          ...
          <div class="p-fluid">
            <div class="p-field">
              ...
              <InputText id="newVin" v-model="newItem.vin" />
            </div>
            <div class="p-field">
              <label for="newBrand">Brand</label>
              <InputText id="newBrand" v-model="newItem.brand" />
            </div>
            <div class="p-field">
              <label for="newYear">Year</label>
              <InputText id="newYear" v-model="newItem.year" />
            </div>
            <div class="p-field">
          ...
          <!-- Botões de ação -->
          <div class="p-dialog-footer">
            <Button label="Cancelar" icon="pi pi-times" class="p-button-text" @click="closeCadastroModal" />
            <Button type="submit" label="Salvar" icon="pi pi-check" class="p-button-text" />
          </div>
        </form>
      </Dialog>
  ...
  const cars = ref([
    { vin: "1234", brand: "Toyota", year: 2020, color: "Blue" },
    { vin: "5678", brand: "Honda", year: 2019, color: "Red" },
    { vin: "9012", brand: "Ford", year: 2021, color: "Black" },
    { vin: "3456", brand: "BMW", year: 2018, color: "White" },
  ]);
